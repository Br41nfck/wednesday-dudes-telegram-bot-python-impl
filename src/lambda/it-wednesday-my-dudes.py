...
async def day_handler(message: types.Message) -> None:
    ...
    image_url = random.choice(IMAGES.get(current_day, IMAGES["Monday"]))
    await message.answer_photo(photo=image_url, caption=f"Today is: {current_day}")


# Add a command to get image for any specific day
@dp.message(Command("getday"))
async def get_specific_day(message: types.Message) -> None:
    """Get image for a specific day."""
    args = message.text.split()
    if len(args) > 1:
        day_name = args[1].capitalize()
        if day_name in IMAGES:
            image_url = random.choice(IMAGES[day_name])
            await message.answer_photo(photo=image_url, caption=f"Image for {day_name}")
        else:
            await message.answer(f"Invalid day. Choose from: {', '.join(IMAGES.keys())}")
    else:
